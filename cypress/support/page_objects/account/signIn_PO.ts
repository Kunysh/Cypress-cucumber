...
class SignIn_PO {
  ...
  _signUpButtonSelector = ".modal-footer .btn-primary";

  get usernameSignUpTextField() {
    return cy.get(
      this._signInModalContainer + " " + this._usernameSignUpTextField
    );
  }

  get passwordSignUpTextField() {
    return cy.get(
      this._signInModalContainer + " " + this._passwordSignUpTextField
    );
  }

  get signUpButtonInRegisterForm() {
    return cy.get(
      this._signInModalContainer + " " + this._signUpButtonSelector
    );
  }
